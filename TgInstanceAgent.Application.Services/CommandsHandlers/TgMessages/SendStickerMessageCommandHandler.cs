...
/// <summary>
/// Обработчик команды отправки стикера.
/// </summary>
...
public class SendStickerMessageCommandHandler(IInstancesService instancesService, IInstanceRepository repository, IMemoryCache cache, IFileStorage fileStorage)
    : IRequestHandler<SendStickerMessageCommand, TgMessage>
{
    /// <summary>
    /// Обработчик команды.
    /// </summary>
    /// <param name="request">Команда отправки стикера.</param>
    /// <param name="cancellationToken">Токен отмены.</param>
    /// <returns>Данные отправленного сообщения</returns>
    public async Task<TgMessage> Handle(SendStickerMessageCommand request, CancellationToken cancellationToken)
    {
        ...
        var message = await service.SendMessageAsync(new TgSendMessageRequest
        {
            ...
            // Данные сообщения
            InputMessageData = new TgInputStickerMessage
            {
                // Устанавливаем файл
